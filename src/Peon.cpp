...
bool Peon::movimientoLegal(Casilla* fin)
{
	...
	if (color == 'B')
	{
		...
		////// PASSANT /////
		// 1 peon B solo puede comer 1 peon N si el peon negro se posiciona al lado de 1 peon B dando 2 pasos -NO 1 SOLO!!-
		// Solo puede comerse el peon N si es el movimiento siguiente al movimiento del peon N
		if (y_fin == 5 && (x_fin == coll + 1 || x_fin == coll - 1) && Tablero::getCasillaOcupada(x_fin, 5) == false && Tablero::getCasillaOcupada(x_fin, 4) == true && Tablero::getCasillaT(x_fin, 4)->getPieza()->getColorPieza() != color && cont_N == 1)
		{
			return true;
		}
